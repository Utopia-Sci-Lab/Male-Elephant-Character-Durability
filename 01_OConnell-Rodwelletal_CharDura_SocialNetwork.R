## ---------------------------
##
## Original script name: calculate-mushara-node-metrics.R
##
...
library(tidyverse)
library(purrr)
library(ggnetwork) # for making figure
library(intergraph) # for making figure
...
ggplot(ggnet_df_v2,
  ...
  geom_nodes(aes(size=eigenvector, col=keystone)) + 
  geom_nodetext(data=ggnet_df_v2, aes(x = x,
                                   y = y,
  ...
  labs(size="Eigenvector centrality",
       ...
       col = "Keystone male?") + 
  theme_blank() +
  scale_colour_manual(values = c("lightskyblue", "gold")) +
